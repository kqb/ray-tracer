...
public:
    ...
    Colour operator *(const Colour& other);
    Colour& operator /(const double d);
    double& operator[](int i);
    double operator[](int i) const;
...
struct Material {
	...
	double exp, int texture_flag, double reflectivity, double transparency) :
	
		ambient(ambient), diffuse(diffuse), specular(specular), 
		specular_exp(exp), tex(texture_flag), reflectivity(reflectivity), transparency(transparency) {}
	...
	// a flag that can tell if a texture is mapped to the object or not
	int tex;
	
	// reflection property of the material 
	double reflectivity;
	
	// transparency property of the material 
    double transparency;
