...
struct SceneDagNode {
    ...
    SceneDagNode() :
    obj(NULL), mat(NULL), next(NULL), parent(NULL), child(NULL) {
    }
...
public:
    ...
    // field of view fov.
    void render(int width, int height, Point3D eye, Vector3D view, Vector3D up,
            double fov, char* fileName);
...
private:
    ...
    Colour refl_refr_count_LessOne(Ray3D& ray, Colour col);
    Colour refl_refr_count_Lesstwo(Ray3D& ray, Colour col);
    ...
    void mapShading(Ray3D& ray, LightListNode* curLight);
